...
//For attributes
app.directive('ngMydemo',function(){
    var direc={};
    var linkFunction = function(scope,element,attributes){
        scope.msg = attributes["ngMydemo"];
    };
    direc.restrict="A";
    direc.link=linkFunction;
    direc.template='<div><h1>{{msg}}</h1></div>';
    direc.scope={};
    
    return direc;
})
//For arrays
app.directive('ngMyarr',function(){
    var direc = {};
    var linkFunction = function(scope,element,attributes){
        scope.items = scope.$eval(attributes.ngMyarr)
    }
    direc.restrict="A";
    direc.link=linkFunction;
    direc.template='<div data-ng-repeat="item in items"><h2>{{item}}</h2></div>'
    direc.scope={};
    
    return direc;
})
//For elements
app.directive('ngMyel',function(){
    var direc = {};
    direc.restrict="E";
    direc.template="<h1>This is how you use a element directive</h1>";
    return direc;
})
