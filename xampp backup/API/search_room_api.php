...
/*
    ...
    */
    ...
    FROM room_types INNER JOIN rates ON room_types.id = rates.room_type_id INNER JOIN room_stocks ON room_types.id = room_stocks.room_type_id WHERE rates.status = 'Approved' AND room_types.guest_max >= $guests AND room_stocks.qty  >= $qty AND room_stocks.range_type = 'All Time' AND rates.per_night_0 > 0 ORDER BY position ASC";
    ...
    ORDER BY rate_start_date ASC";
        ...
        $extras = $pamconnection->query($query);
        $pattern = "/\b".$room_id."\b/";
        $extras_array = array();
